...
- `useInteractions` (custom hook to track interactions on Landing Page and Customer Reviews)  
...
- Only handles data access – it does not perform validation or manage UI state.  
...
- Indirectly used in `Body.tsx` and `AllCommentsPage.tsx` through the service and hook.
...
- Used in `Body.tsx` and `AllCommentsPage.tsx` indirectly via the hook.
...
**Where is it used?**  
- `Body.tsx` – manages comments in the main page.  
- `AllCommentsPage.tsx` – displays and allows modification of all comments.
...
- Total reviews submitted on Customer Reviews page  
